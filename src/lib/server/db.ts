...
export async function generateInviteCode(
  ...
  await db
    .prepare('INSERT INTO invite_codes (id, code, family_id, created_by, expires_at) VALUES (?, ?, ?, ?, ?)')
...
export async function getInviteCodeByCode(db: D1Database, code: string): Promise<InviteCode | null> {
  return db.prepare('SELECT * FROM invite_codes WHERE code = ?').bind(code.toUpperCase()).first<InviteCode>();
}

export async function getActiveInviteCode(db: D1Database, familyId: string): Promise<InviteCode | null> {
  const now = Math.floor(Date.now() / 1000);
  return db
    .prepare('SELECT * FROM invite_codes WHERE family_id = ? AND used_by IS NULL AND expires_at > ? ORDER BY created_at DESC LIMIT 1')
    .bind(familyId, now)
    .first<InviteCode>();
}
...
export async function updateUserFamily(db: D1Database, userId: string, familyId: string): Promise<void> {
...
export async function getSettings(db: D1Database, familyId: string): Promise<Settings> {
  const result = await db.prepare('SELECT * FROM settings WHERE family_id = ?').bind(familyId).first<Settings>();
  if (!result) {
    await db.prepare('INSERT INTO settings (family_id) VALUES (?)').bind(familyId).run();
    return { id: 0, family_id: familyId, currency: 'EUR', pin_enabled: 0, pin_hash: null, pin_timeout_minutes: 1 };
  }
  return result;
}
...
export async function updateSettings(
  ...
  await db
    .prepare(
      'UPDATE settings SET currency = ?, pin_enabled = ?, pin_hash = ?, pin_timeout_minutes = ? WHERE family_id = ?'
    )
    .bind(
...
export async function createUser(
  ...
  await db
    .prepare('INSERT INTO users (id, email, password_hash, name, photo_url, family_id) VALUES (?, ?, ?, ?, ?, ?)')
...
export async function getChildren(db: D1Database, familyId: string): Promise<Child[]> {
  const result = await db.prepare('SELECT * FROM children WHERE family_id = ? ORDER BY sort_order, created_at').bind(familyId).all<Child>();
  return result.results;
}
...
export async function createChild(db: D1Database, child: Omit<Child, 'created_at' | 'photo_data'>): Promise<void> {
  await db
    .prepare('INSERT INTO children (id, name, color, photo_url, family_id, sort_order) VALUES (?, ?, ?, ?, ?, ?)')
...
export async function updateChild(
  ...
  if (!child) return;
  await db
    .prepare('UPDATE children SET name = ?, color = ?, photo_url = ?, photo_data = ?, sort_order = ? WHERE id = ?')
    .bind(
...
export async function getTransactions(
  ...
  childId: string,
  limit = 50
): Promise<(Transaction & { user_name?: string; user_photo_url?: string | null; user_photo_data?: string | null })[]> {
  const result = await db
    .prepare(
    ...
    .bind(childId, limit)
    .all<Transaction & { user_name?: string; user_photo_url?: string | null; user_photo_data?: string | null }>();
  return result.results;
}
...
export async function getBalanceHistory(
  ...
  for (const row of result.results) {
    runningBalance = startBalance + row.running_balance - (result.results[0]?.running_balance ?? 0) + (result.results[0]?.running_balance ?? 0);
