...
<div class="space-y-6">
    ...
    >
      <div class="flex gap-4 items-end">
        <div class="flex-1">
          ...
          <select id="currency" name="currency" class="input" value={data.settings?.currency ?? 'EUR'}>
            ...
            {#each data.familyMembers as member}
              <div class="flex items-center gap-3 p-2 rounded-lg bg-gray-50">
                {#if member.photo_data}
                  <img src={member.photo_data} alt={member.name} class="w-8 h-8 rounded-full object-cover" />
                {:else if member.photo_url}
                  <img src={member.photo_url} alt={member.name} class="w-8 h-8 rounded-full object-cover" />
                {:else}
                  <div class="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center">
                    <span class="text-sm font-medium text-blue-600">{member.name.charAt(0).toUpperCase()}</span>
                  </div>
                {/if}
                <div>
  ...
  <div class="card p-6">
    ...
    <p class="text-sm text-gray-500 mb-4">
      Add an extra layer of security by requiring a PIN to access the app after a period of inactivity.
    </p>
    ...
    {#if data.settings?.pin_enabled}
      <div class="space-y-4">
        ...
        <form
          method="POST"
          action="?/updatePinTimeout"
          use:enhance
          class="flex gap-4 items-end"
        >
          <div class="flex-1">
            <label for="timeout" class="label">PIN Timeout</label>
            <select id="timeout" name="timeout" class="input" value={data.settings.pin_timeout_minutes}>
      ...
      <div class="relative w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        ...
        >
          ...
          <div class="flex justify-end gap-3 pt-4">
            ...
            <button type="submit" class={pinAction === 'disable' ? 'btn-danger' : 'btn-primary'} disabled={loading}>
              {#if loading}
                Processing...
