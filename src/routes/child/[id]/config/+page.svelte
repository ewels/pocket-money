            ...
            <div class="flex gap-1">
              ...
              {#if rule.active}
                <form method="POST" action="?/toggleSkip" use:enhance>
                  ...
                  <button
                    type="submit"
                    class="p-2 {rule.skip_next ? 'text-orange-500' : 'text-gray-400'} hover:text-gray-600"
...
{#if showAddTarget}
  <div class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex min-h-full items-center justify-center p-4">
      <button type="button" class="fixed inset-0 bg-black/50" onclick={() => (showAddTarget = false)} aria-label="Close"></button>
      <div class="relative w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        ...
        >
          <div>
            ...
            <input id="targetName" name="name" type="text" required class="input" placeholder="e.g., New bike" />
          </div>
          <div>
            <label for="targetAmount" class="label">Target Amount</label>
            <input id="targetAmount" name="amount" type="number" step="0.01" min="0.01" required class="input" placeholder="0.00" />
          </div>
          <div class="flex justify-end gap-3 pt-4">
            <button type="button" class="btn-secondary" onclick={() => (showAddTarget = false)}>Cancel</button>
...
{#if showAddRecurring}
  <div class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex min-h-full items-center justify-center p-4">
      <button type="button" class="fixed inset-0 bg-black/50" onclick={() => (showAddRecurring = false)} aria-label="Close"></button>
      <div class="relative w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        ...
        >
          <div>
            ...
            <input id="recurringAmount" name="amount" type="number" step="0.01" min="0.01" required class="input" placeholder="0.00" />
          </div>
          <div>
            <label for="recurringDescription" class="label">Description (optional)</label>
            <input id="recurringDescription" name="description" type="text" class="input" placeholder="e.g., Weekly allowance" />
          </div>
          <div>
          ...
          <div class="flex justify-end gap-3 pt-4">
            <button type="button" class="btn-secondary" onclick={() => (showAddRecurring = false)}>Cancel</button>
...
{#if editingTarget}
  ...
  {#if target}
    <div class="fixed inset-0 z-50 overflow-y-auto">
      <div class="flex min-h-full items-center justify-center p-4">
        <button type="button" class="fixed inset-0 bg-black/50" onclick={() => (editingTarget = null)} aria-label="Close"></button>
        <div class="relative w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
          ...
          >
            ...
            <div>
              ...
              <input id="editTargetName" name="name" type="text" required class="input" value={target.name} />
            </div>
            <div>
              <label for="editTargetAmount" class="label">Target Amount</label>
              <input id="editTargetAmount" name="amount" type="number" step="0.01" min="0.01" required class="input" value={target.target_amount} />
            </div>
            <div class="flex justify-end gap-3 pt-4">
              <button type="button" class="btn-secondary" onclick={() => (editingTarget = null)}>Cancel</button>
...
{#if showDeleteConfirm}
  <div class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex min-h-full items-center justify-center p-4">
      <button type="button" class="fixed inset-0 bg-black/50" onclick={() => (showDeleteConfirm = false)} aria-label="Close"></button>
      <div class="relative w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        <h2 class="text-lg font-semibold text-red-600">Delete {data.child.name}?</h2>
        <p class="mt-2 text-sm text-gray-500">
          This will permanently delete {data.child.name} and all their transaction history.
